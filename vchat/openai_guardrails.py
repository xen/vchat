...
def _config_path() -> Path:
    raw = str(config.get("guardrails_config_path", "guardrails_config.json"))
    path = Path(raw)
...
def get_guardrails_client(*, api_key: str, base_url: str) -> GuardrailsAsyncOpenAI | None:
    if not bool(config.get("openai_guardrails_enabled", True)):
        return None

    cfg_path = _config_path()
    if not cfg_path.exists():
        logger.warning("Guardrails config not found at '%s', guardrails disabled", cfg_path)
        return None
...
def extract_tripwire_details(exc: GuardrailTripwireTriggered) -> tuple[str, str]:
    info: dict[str, Any] = getattr(getattr(exc, "guardrail_result", None), "info", {}) or {}
