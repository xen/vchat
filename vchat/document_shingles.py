...
def find_repeated_shingles(docs: List[str], k: int = 10, min_freq: float = 0.5) -> List[str]:
...
def visualize_removed_blocks(removed: List[str]) -> str:
    ...
    blocks = "".join(f'<div class="bg-warning/20 border border-warning rounded p-2 my-1 text-xs">{re.escape(block[:200])}...</div>' for block in removed)
