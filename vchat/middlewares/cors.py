...
#     from .middlewares.cors import DEFAULT_ALLOW_HEADERS
...
DEFAULT_ALLOW_METHODS = ("DELETE", "GET", "OPTIONS", "PATCH", "POST", "PUT", "HEAD")
...
def cors_middleware(
    *,
    ...
    """
    check_urls = DEFAULT_URLS if urls is None else urls
    ...
    async def middleware(request, handler):
        ...
        is_preflight_request = (
            is_options_request and ACCESS_CONTROL_REQUEST_METHOD in request.headers
        )
        ...
        if not allow_all and not (origins and match_items(origins, origin)):
            logger.debug("CORS headers not allowed for given Origin", extra=log_extra)
            return response
        ...
        if expose_headers:
            response.headers[ACCESS_CONTROL_EXPOSE_HEADERS] = ", ".join(expose_headers)

        # Now, if this is an options request, respond with extra Allow headers
        if is_options_request:
            response.headers[ACCESS_CONTROL_ALLOW_HEADERS] = ", ".join(allow_headers)
            response.headers[ACCESS_CONTROL_ALLOW_METHODS] = ", ".join(allow_methods)
        ...
        if is_preflight_request:
            logger.debug(
                "Provide CORS headers with empty response for preflight request",
                extra=log_extra,
            )
