...
BEGIN
  ...
  --raise notice 'num passed to toId: %', num;
  --raise notice 'alphabet passed to toId: %', alphabet;
...
BEGIN
  ...
  IF sqids.isBlockedId(id) THEN
    --raise notice 'Blocked ID: %', id;
    id := sqids.encodeNumbers(numbers, original_alphabet, minLength, increment + 1);
  END IF;
