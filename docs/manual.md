...
| ------------------------------------------------------------------------- | --------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Промышленные секреты: `SECRET_KEY`, `COOKIE_KEY`, учетные данные БД и LLM | Секрет                                  | Kubernetes `Secret`, внешний менеджер секретов, переменные окружения                   | Не хранить в Git и образе; ограничить RBAC; ротировать при компрометации; не логировать.                                                                     |
...
В `mode: production` приложение не стартует, если `secret_key` или
`cookie_key` остались дефолтными, пустыми или равны `change-me`.
...
приложение не стартует, если `secret_key` или `cookie_key` пустые, дефолтные
или равны `change-me`.
...
| `vchat_request_embedding_queue_wait_seconds` | Ожидание локальной очереди request embedding внутри web process | p95 близко к `request_embedding_queue_timeout_seconds` означает нехватку web capacity  |
| `vchat_request_embedding_encode_seconds`     | Время одного `SentenceTransformer.encode()` в request path      | Рост p95 означает CPU contention, неподходящее число torch threads или деградацию host |
| `vchat_request_embedding_inflight`           | Сколько request embeddings сейчас выполняется                   | Для одного web process штатно `0..1`; выше требует отдельного benchmark                |
| `vchat_request_embedding_timeouts_total`     | Сколько запросов не дождались локальной очереди embedding       | Любой рост требует уменьшить нагрузку на pod или добавить web capacity                 |
...
#### Управление масштабированием request embeddings

Request-path embedding считается внутри `vchat-web`, но в каждом web process
должен выполняться только один активный `SentenceTransformer.encode()`.
Параллелизм достигается количеством web pod/processes, а не увеличением thread
pool внутри одного процесса.

Базовая конфигурация web process:

```yaml
request_embedding_concurrency: 1
request_embedding_executor_workers: 1
request_embedding_queue_timeout_seconds: 20
request_embedding_torch_threads: 1
```

Для CPU-only сервера повышайте только `request_embedding_torch_threads` и число
web replicas. Значение `request_embedding_torch_threads` должно соответствовать
выделенным CPU для pod/process: для pod с лимитом `2` CPU используйте `2`, для
выделенного bare-metal процесса подбирайте значение нагрузочным тестом.

Что наблюдать:

| Симптом                                                 | Действие                                                                            |
| ------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Растет `vchat_request_embedding_queue_wait_seconds`     | Добавить web replica/process или снизить входной concurrency                        |
| Растет `vchat_request_embedding_timeouts_total`         | Увеличить web capacity; не поднимать `request_embedding_concurrency` без benchmark  |
| Растет `vchat_request_embedding_encode_seconds`         | Проверить CPU throttling, `OMP_NUM_THREADS`/`MKL_NUM_THREADS`, memory pressure      |
| RSS web process приближается к limit                    | Увеличить memory limit или уменьшить число pod на host                              |
| p95 ответа ниже требований только при малом concurrency | Выносить request embeddings в отдельный controlled service/worker отдельной задачей |

Практическая стратегия:

1. Начать с `replicas=1`, `request_embedding_concurrency=1`,
   `request_embedding_executor_workers=1`.
2. Задать `request_embedding_torch_threads` равным CPU limit pod.
3. Проверить p95 `vchat_request_embedding_queue_wait_seconds` и
   `vchat_request_embedding_encode_seconds` под ожидаемым concurrency.
4. Если очередь растет, добавить web replica/process и держать
   `request_embedding_concurrency=1`.
5. Если после добавления replicas растут encode latency и RSS, host уперся в CPU
   или память; дальше масштабировать на другой host, а не внутри процесса.
